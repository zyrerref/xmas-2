...
function playChime(){
  // WebAudio: short "Christmas-ish" triad chime
...
function setMusic(on){
  ...
  if (on){
    // Some browsers require a user gesture; this is called by clicking the button, so OK.
    playChime();
...
function updateShareLink(){
  ...
  if (toName.value.trim()) params.set("to", toName.value.trim());
  if (fromName.value.trim()) params.set("from", fromName.value.trim());
  if (msg.value.trim()) params.set("msg", msg.value.trim());

  // keep theme + snow preference shareable too
  params.set("theme", document.documentElement.dataset.theme || "dark");
  params.set("snow", state.snowOn ? "1" : "0");
...
function readFromURL(){
  ...
  if (snow === "0" || snow === "1") setSnow(snow === "1");
}
...
nativeShareBtn.addEventListener("click", async () => {
  ...
  if (navigator.share){
    ...
    // No native share support; just copy link
...
// If shared link turned snow off, keep snow array still ready
